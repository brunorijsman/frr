...
DEFINE_MTYPE_STATIC(RIFTD, RIFT_TLV, "RIFT TLVs")
DEFINE_MTYPE_STATIC(RIFTD, RIFT_SUBTLV, "RIFT Sub-TLVs")
DEFINE_MTYPE_STATIC(RIFTD, RIFT_MT_ITEM_LIST, "RIFT MT Item Lists")

typedef int (*unpack_tlv_func)(enum rift_tlv_context context, uint8_t tlv_type,
			       uint8_t tlv_len, struct stream *s,
			       struct sbuf *log, void *dest, int indent);
typedef int (*pack_item_func)(struct rift_item *item, struct stream *s);
typedef void (*free_item_func)(struct rift_item *i);
typedef int (*unpack_item_func)(uint16_t mtid, uint8_t len, struct stream *s,
				struct sbuf *log, void *dest, int indent);
typedef void (*format_item_func)(uint16_t mtid, struct rift_item *i,
				 struct sbuf *buf, int indent);
typedef struct rift_item *(*copy_item_func)(struct rift_item *i);
...
struct pack_order_entry {
	...
	enum rift_tlv_type type;
	enum how_to_pack how_to_pack;
	size_t what_to_pack;
};
#define PACK_ENTRY(t, h, w)                                                    \
	{                                                                      \
		.context = RIFT_CONTEXT_LSP, .type = RIFT_TLV_##t,             \
		.how_to_pack = (h),                                            \
		.what_to_pack = offsetof(struct rift_tlvs, w),                 \
	}

static struct pack_order_entry pack_order[] = {
	PACK_ENTRY(OLDSTYLE_REACH, RIFT_ITEMS, oldstyle_reach),
	PACK_ENTRY(LAN_NEIGHBORS, RIFT_ITEMS, lan_neighbor),
	PACK_ENTRY(LSP_ENTRY, RIFT_ITEMS, lsp_entries),
	PACK_ENTRY(EXTENDED_REACH, RIFT_ITEMS, extended_reach),
	PACK_ENTRY(MT_REACH, RIFT_MT_ITEMS, mt_reach),
	PACK_ENTRY(OLDSTYLE_IP_REACH, RIFT_ITEMS, oldstyle_ip_reach),
	PACK_ENTRY(OLDSTYLE_IP_REACH_EXT, RIFT_ITEMS, oldstyle_ip_reach_ext),
	PACK_ENTRY(IPV4_ADDRESS, RIFT_ITEMS, ipv4_address),
	PACK_ENTRY(IPV6_ADDRESS, RIFT_ITEMS, ipv6_address),
	PACK_ENTRY(EXTENDED_IP_REACH, RIFT_ITEMS, extended_ip_reach),
	PACK_ENTRY(MT_IP_REACH, RIFT_MT_ITEMS, mt_ip_reach),
	PACK_ENTRY(IPV6_REACH, RIFT_ITEMS, ipv6_reach),
	PACK_ENTRY(MT_IPV6_REACH, RIFT_MT_ITEMS, mt_ipv6_reach)};

/* This is a forward definition. The table is actually initialized
 * in at the bottom. */
static const struct tlv_ops *tlv_table[RIFT_CONTEXT_MAX][RIFT_TLV_MAX];

/* End of _ops forward definition. */

/* Prototypes */
static void append_item(struct rift_item_list *dest, struct rift_item *item);

/* Functions for Sub-TLV 3 SR Prefix-SID */

static struct rift_item *copy_item_prefix_sid(struct rift_item *i)
{
	struct rift_prefix_sid *sid = (struct rift_prefix_sid *)i;
	struct rift_prefix_sid *rv = XCALLOC(MTYPE_RIFT_SUBTLV, sizeof(*rv));

	rv->flags = sid->flags;
	rv->algorithm = sid->algorithm;
	rv->value = sid->value;
	return (struct rift_item *)rv;
}

static void format_item_prefix_sid(uint16_t mtid, struct rift_item *i,
				   struct sbuf *buf, int indent)
{
	struct rift_prefix_sid *sid = (struct rift_prefix_sid *)i;
	...
	sbuf_push(buf, indent, "  Flags:%s%s%s%s%s%s\n",
		  sid->flags & RIFT_PREFIX_SID_READVERTISED ? " READVERTISED" : "",
		  sid->flags & RIFT_PREFIX_SID_NODE ? " NODE" : "",
		  sid->flags & RIFT_PREFIX_SID_NO_PHP ? " NO_PHP" : "",
		  sid->flags & RIFT_PREFIX_SID_EXPLICIT_NULL ? " EXPLICIT-NULL" : "",
		  sid->flags & RIFT_PREFIX_SID_VALUE ? " VALUE" : "",
		  sid->flags & RIFT_PREFIX_SID_LOCAL ? " LOCAL" : "");
	sbuf_push(buf, indent, "  Algorithm: %" PRIu8 "\n", sid->algorithm);
	if (sid->flags & RIFT_PREFIX_SID_VALUE) {
...
static void free_item_prefix_sid(struct rift_item *i)
{
	XFREE(MTYPE_RIFT_SUBTLV, i);
}

static int pack_item_prefix_sid(struct rift_item *i, struct stream *s)
{
	struct rift_prefix_sid *sid = (struct rift_prefix_sid *)i;

	uint8_t size = (sid->flags & RIFT_PREFIX_SID_VALUE) ? 5 : 6;
...
static int unpack_item_prefix_sid(uint16_t mtid, uint8_t len, struct stream *s,
				  struct sbuf *log, void *dest, int indent)
{
	struct rift_subtlvs *subtlvs = dest;
	struct rift_prefix_sid sid = {
	};
	...
	if ((sid.flags & RIFT_PREFIX_SID_VALUE)
	    != (sid.flags & RIFT_PREFIX_SID_LOCAL)) {
	...
	uint8_t expected_size = (sid.flags & RIFT_PREFIX_SID_VALUE) ? 5 : 6;
	...
	format_item_prefix_sid(mtid, (struct rift_item *)&sid, log, indent + 2);
	append_item(&subtlvs->prefix_sids, copy_item_prefix_sid((struct rift_item *)&sid));
	return 0;
}
...
static struct prefix_ipv6 *copy_subtlv_ipv6_source_prefix(struct prefix_ipv6 *p)
{
	...
	struct prefix_ipv6 *rv = XCALLOC(MTYPE_RIFT_SUBTLV, sizeof(*rv));
	rv->family = p->family;
					  ...
					  struct stream *s)
{
	...
	stream_putc(s, RIFT_SUBTLV_IPV6_SOURCE_PREFIX);
...
static int unpack_subtlv_ipv6_source_prefix(enum rift_tlv_context context,
					    ...
					    void *dest, int indent)
{
	...
	subtlvs->source_prefix = XCALLOC(MTYPE_RIFT_SUBTLV, sizeof(p));
	memcpy(subtlvs->source_prefix, &p, sizeof(p));
	return 0;
}
static void init_item_list(struct rift_item_list *items);
static struct rift_item *copy_item(enum rift_tlv_context context,
				   enum rift_tlv_type type,
				   struct rift_item *item);
static void copy_items(enum rift_tlv_context context, enum rift_tlv_type type,
		       struct rift_item_list *src, struct rift_item_list *dest);
static void format_items_(uint16_t mtid, enum rift_tlv_context context,
			  enum rift_tlv_type type, struct rift_item_list *items,
			  struct sbuf *buf, int indent);
#define format_items(...) format_items_(RIFT_MT_IPV4_UNICAST, __VA_ARGS__)
static void free_items(enum rift_tlv_context context, enum rift_tlv_type type,
		       struct rift_item_list *items);
static int pack_items_(uint16_t mtid, enum rift_tlv_context context,
		       enum rift_tlv_type type, struct rift_item_list *items,
		       struct stream *s, struct rift_tlvs **fragment_tlvs,
		       struct pack_order_entry *pe,
		       struct rift_tlvs *(*new_fragment)(struct list *l),
		       struct list *new_fragment_arg);
#define pack_items(...) pack_items_(RIFT_MT_IPV4_UNICAST, __VA_ARGS__)

/* Functions related to subtlvs */

static struct rift_subtlvs *rift_alloc_subtlvs(enum rift_tlv_context context)
{
	struct rift_subtlvs *result;

	result = XCALLOC(MTYPE_RIFT_SUBTLV, sizeof(*result));
...
static struct rift_subtlvs *copy_subtlvs(struct rift_subtlvs *subtlvs)
{
	if (!subtlvs)
		return NULL;

	struct rift_subtlvs *rv = XCALLOC(MTYPE_RIFT_SUBTLV, sizeof(*rv));

	rv->context = subtlvs->context;

	copy_items(subtlvs->context, RIFT_SUBTLV_PREFIX_SID,
...
static void format_subtlvs(struct rift_subtlvs *subtlvs, struct sbuf *buf,
			   int indent)
{
	format_items(subtlvs->context, RIFT_SUBTLV_PREFIX_SID,
		     &subtlvs->prefix_sids, buf, indent);

	format_subtlv_ipv6_source_prefix(subtlvs->source_prefix, buf, indent);
}

static void rift_free_subtlvs(struct rift_subtlvs *subtlvs)
{
	if (!subtlvs)
		return;

	free_items(subtlvs->context, RIFT_SUBTLV_PREFIX_SID,
		   &subtlvs->prefix_sids);

	XFREE(MTYPE_RIFT_SUBTLV, subtlvs->source_prefix);

	XFREE(MTYPE_RIFT_SUBTLV, subtlvs);
}

static int pack_subtlvs(struct rift_subtlvs *subtlvs, struct stream *s)
{
	int rv;
	...
	rv = pack_items(subtlvs->context, RIFT_SUBTLV_PREFIX_SID,
...
static int unpack_tlvs(enum rift_tlv_context context, size_t avail_len,
		       struct stream *stream, struct sbuf *log, void *dest,
		       int indent);

/* Functions related to TLVs 1 Area Addresses */

static struct rift_item *copy_item_area_address(struct rift_item *i)
{
	struct rift_area_address *addr = (struct rift_area_address *)i;
	struct rift_area_address *rv = XCALLOC(MTYPE_RIFT_TLV, sizeof(*rv));
...
static void format_item_area_address(uint16_t mtid, struct rift_item *i,
				     struct sbuf *buf, int indent)
{
	struct rift_area_address *addr = (struct rift_area_address *)i;
...
static void free_item_area_address(struct rift_item *i)
{
	XFREE(MTYPE_RIFT_TLV, i);
}

static int pack_item_area_address(struct rift_item *i, struct stream *s)
{
	struct rift_area_address *addr = (struct rift_area_address *)i;
...
static int unpack_item_area_address(uint16_t mtid, uint8_t len,
				    ...
				    void *dest, int indent)
{
	struct rift_tlvs *tlvs = dest;
	struct rift_area_address *rv = NULL;
	...
	rv = XCALLOC(MTYPE_RIFT_TLV, sizeof(*rv));
	...
	format_item_area_address(RIFT_MT_IPV4_UNICAST, (struct rift_item *)rv,
				 log, indent + 2);
	append_item(&tlvs->area_addresses, (struct rift_item *)rv);
	return 0;
out:
	XFREE(MTYPE_RIFT_TLV, rv);
	return 1;
}

/* Functions related to TLV 2 (Old-Style) IS Reach */
static struct rift_item *copy_item_oldstyle_reach(struct rift_item *i)
{
	struct rift_oldstyle_reach *r = (struct rift_oldstyle_reach *)i;
	struct rift_oldstyle_reach *rv = XCALLOC(MTYPE_RIFT_TLV, sizeof(*rv));
...
static void format_item_oldstyle_reach(uint16_t mtid, struct rift_item *i,
				       struct sbuf *buf, int indent)
{
	struct rift_oldstyle_reach *r = (struct rift_oldstyle_reach *)i;

	sbuf_push(buf, indent, "IS Reachability: %s (Metric: %" PRIu8 ")\n",
		  rift_format_id(r->id, 7), r->metric);
}

static void free_item_oldstyle_reach(struct rift_item *i)
{
	XFREE(MTYPE_RIFT_TLV, i);
}

static int pack_item_oldstyle_reach(struct rift_item *i, struct stream *s)
{
	struct rift_oldstyle_reach *r = (struct rift_oldstyle_reach *)i;
				      ...
				      void *dest, int indent)
{
	...
	format_item_oldstyle_reach(mtid, (struct rift_item *)rv, log,
				   indent + 2);
	append_item(&tlvs->oldstyle_reach, (struct rift_item *)rv);
	return 0;
}

/* Functions related to TLV 6 LAN Neighbors */
static struct rift_item *copy_item_lan_neighbor(struct rift_item *i)
{
	struct rift_lan_neighbor *n = (struct rift_lan_neighbor *)i;
	struct rift_lan_neighbor *rv = XCALLOC(MTYPE_RIFT_TLV, sizeof(*rv));

	memcpy(rv->mac, n->mac, 6);
	return (struct rift_item *)rv;
}

static void format_item_lan_neighbor(uint16_t mtid, struct rift_item *i,
				     struct sbuf *buf, int indent)
{
	struct rift_lan_neighbor *n = (struct rift_lan_neighbor *)i;

	sbuf_push(buf, indent, "LAN Neighbor: %s\n", rift_format_id(n->mac, 6));
}

static void free_item_lan_neighbor(struct rift_item *i)
{
	XFREE(MTYPE_RIFT_TLV, i);
}

static int pack_item_lan_neighbor(struct rift_item *i, struct stream *s)
{
	struct rift_lan_neighbor *n = (struct rift_lan_neighbor *)i;
				    ...
				    void *dest, int indent)
{
	...
	format_item_lan_neighbor(mtid, (struct rift_item *)rv, log, indent + 2);
	append_item(&tlvs->lan_neighbor, (struct rift_item *)rv);
	return 0;
}

/* Functions related to TLV 9 LSP Entry */
static struct rift_item *copy_item_lsp_entry(struct rift_item *i)
{
	struct rift_lsp_entry *e = (struct rift_lsp_entry *)i;
	struct rift_lsp_entry *rv = XCALLOC(MTYPE_RIFT_TLV, sizeof(*rv));
...
static void format_item_lsp_entry(uint16_t mtid, struct rift_item *i,
				  struct sbuf *buf, int indent)
{
	struct rift_lsp_entry *e = (struct rift_lsp_entry *)i;
	...
	sbuf_push(buf, indent,
		  ...
		  ", lifetime %" PRIu16 "s\n",
		  rift_format_id(e->id, 8), e->seqno, e->checksum,
		  e->rem_lifetime);
}

static void free_item_lsp_entry(struct rift_item *i)
{
	XFREE(MTYPE_RIFT_TLV, i);
}

static int pack_item_lsp_entry(struct rift_item *i, struct stream *s)
{
	struct rift_lsp_entry *e = (struct rift_lsp_entry *)i;
				 ...
				 struct sbuf *log, void *dest, int indent)
{
	...
	format_item_lsp_entry(mtid, (struct rift_item *)rv, log, indent + 2);
	append_item(&tlvs->lsp_entries, (struct rift_item *)rv);
	return 0;
}

/* Functions related to TLVs 22/222 Extended Reach/MT Reach */

static struct rift_item *copy_item_extended_reach(struct rift_item *i)
{
	struct rift_extended_reach *r = (struct rift_extended_reach *)i;
	struct rift_extended_reach *rv = XCALLOC(MTYPE_RIFT_TLV, sizeof(*rv));

	memcpy(rv->id, r->id, 7);
	rv->metric = r->metric;

	if (r->subtlvs && r->subtlv_len) {
		rv->subtlvs = XCALLOC(MTYPE_RIFT_TLV, r->subtlv_len);
...
static void format_item_extended_reach(uint16_t mtid, struct rift_item *i,
				       struct sbuf *buf, int indent)
{
	struct rift_extended_reach *r = (struct rift_extended_reach *)i;

	sbuf_push(buf, indent, "%s Reachability: %s (Metric: %u)",
		  (mtid == RIFT_MT_IPV4_UNICAST) ? "Extended" : "MT",
		  rift_format_id(r->id, 7), r->metric);
	if (mtid != RIFT_MT_IPV4_UNICAST)
		sbuf_push(buf, 0, " %s", rift_mtid2str(mtid));
	sbuf_push(buf, 0, "\n");
...
static void free_item_extended_reach(struct rift_item *i)
{
	struct rift_extended_reach *item = (struct rift_extended_reach *)i;
	XFREE(MTYPE_RIFT_TLV, item->subtlvs);
	XFREE(MTYPE_RIFT_TLV, item);
}

static int pack_item_extended_reach(struct rift_item *i, struct stream *s)
{
	struct rift_extended_reach *r = (struct rift_extended_reach *)i;
...
{
	struct rift_tlvs *tlvs = dest;
	struct rift_extended_reach *rv = NULL;
	uint8_t subtlv_len;
	struct rift_item_list *items;
	...
	if (mtid == RIFT_MT_IPV4_UNICAST) {
		...
		items = rift_get_mt_items(&tlvs->mt_reach, mtid);
	}
	...
	format_item_extended_reach(mtid, (struct rift_item *)rv, log,
				   indent + 2);
	...
	if (subtlv_len) {
		...
		if (unpack_tlvs(RIFT_CONTEXT_SUBTLV_NE_REACH, subtlv_len, s,
				log, NULL, indent + 4)) {
			goto out;
		}
		...
		rv->subtlvs = XCALLOC(MTYPE_RIFT_TLV, subtlv_len);
		stream_get(rv->subtlvs, s, subtlv_len);
		rv->subtlv_len = subtlv_len;
	}

	append_item(items, (struct rift_item *)rv);
	return 0;
out:
	if (rv)
		free_item_extended_reach((struct rift_item *)rv);

	return 1;
}

/* Functions related to TLV 128 (Old-Style) IP Reach */
static struct rift_item *copy_item_oldstyle_ip_reach(struct rift_item *i)
{
	struct rift_oldstyle_ip_reach *r = (struct rift_oldstyle_ip_reach *)i;
	struct rift_oldstyle_ip_reach *rv =
		XCALLOC(MTYPE_RIFT_TLV, sizeof(*rv));
...
static void format_item_oldstyle_ip_reach(uint16_t mtid, struct rift_item *i,
					  struct sbuf *buf, int indent)
{
	struct rift_oldstyle_ip_reach *r = (struct rift_oldstyle_ip_reach *)i;
	char prefixbuf[PREFIX2STR_BUFFER];
...
static void free_item_oldstyle_ip_reach(struct rift_item *i)
{
	XFREE(MTYPE_RIFT_TLV, i);
}

static int pack_item_oldstyle_ip_reach(struct rift_item *i, struct stream *s)
{
	struct rift_oldstyle_ip_reach *r = (struct rift_oldstyle_ip_reach *)i;
					 ...
					 void *dest, int indent)
{
	...
	format_item_oldstyle_ip_reach(mtid, (struct rift_item *)rv, log,
				      indent + 2);
	append_item(dest, (struct rift_item *)rv);
	return 0;
}
...
static void copy_tlv_protocols_supported(struct rift_protocols_supported *src,
					 struct rift_protocols_supported *dest)
{
	if (!src->protocols || !src->count)
		return;
	dest->count = src->count;
	dest->protocols = XCALLOC(MTYPE_RIFT_TLV, src->count);
	memcpy(dest->protocols, src->protocols, src->count);
}

static void format_tlv_protocols_supported(struct rift_protocols_supported *p,
					   struct sbuf *buf, int indent)
{
...
static void free_tlv_protocols_supported(struct rift_protocols_supported *p)
{
	XFREE(MTYPE_RIFT_TLV, p->protocols);
}

static int pack_tlv_protocols_supported(struct rift_protocols_supported *p,
					struct stream *s)
{
	if (!p || !p->count || !p->protocols)
		return 0;
	...
	stream_putc(s, RIFT_TLV_PROTOCOLS_SUPPORTED);
	stream_putc(s, p->count);
	stream_put(s, p->protocols, p->count);
	return 0;
}

static int unpack_tlv_protocols_supported(enum rift_tlv_context context,
					  uint8_t tlv_type, uint8_t tlv_len,
					  struct stream *s, struct sbuf *log,
					  void *dest, int indent)
{
	...
	tlvs->protocols_supported.protocols = XCALLOC(MTYPE_RIFT_TLV, tlv_len);
...
static struct rift_item *copy_item_ipv4_address(struct rift_item *i)
{
	struct rift_ipv4_address *a = (struct rift_ipv4_address *)i;
	struct rift_ipv4_address *rv = XCALLOC(MTYPE_RIFT_TLV, sizeof(*rv));

	rv->addr = a->addr;
	return (struct rift_item *)rv;
}

static void format_item_ipv4_address(uint16_t mtid, struct rift_item *i,
				     struct sbuf *buf, int indent)
{
	struct rift_ipv4_address *a = (struct rift_ipv4_address *)i;
...
static void free_item_ipv4_address(struct rift_item *i)
{
	XFREE(MTYPE_RIFT_TLV, i);
}

static int pack_item_ipv4_address(struct rift_item *i, struct stream *s)
{
	struct rift_ipv4_address *a = (struct rift_ipv4_address *)i;
				    ...
				    void *dest, int indent)
{
	...
	format_item_ipv4_address(mtid, (struct rift_item *)rv, log, indent + 2);
	append_item(&tlvs->ipv4_address, (struct rift_item *)rv);
	return 0;
}


/* Functions related to TLV 232 IPv6 Interface addresses */
static struct rift_item *copy_item_ipv6_address(struct rift_item *i)
{
	struct rift_ipv6_address *a = (struct rift_ipv6_address *)i;
	struct rift_ipv6_address *rv = XCALLOC(MTYPE_RIFT_TLV, sizeof(*rv));

	rv->addr = a->addr;
	return (struct rift_item *)rv;
}

static void format_item_ipv6_address(uint16_t mtid, struct rift_item *i,
				     struct sbuf *buf, int indent)
{
	struct rift_ipv6_address *a = (struct rift_ipv6_address *)i;
...
static void free_item_ipv6_address(struct rift_item *i)
{
	XFREE(MTYPE_RIFT_TLV, i);
}

static int pack_item_ipv6_address(struct rift_item *i, struct stream *s)
{
	struct rift_ipv6_address *a = (struct rift_ipv6_address *)i;
				    ...
				    void *dest, int indent)
{
	...
	format_item_ipv6_address(mtid, (struct rift_item *)rv, log, indent + 2);
	append_item(&tlvs->ipv6_address, (struct rift_item *)rv);
	return 0;
}


/* Functions related to TLV 229 MT Router information */
static struct rift_item *copy_item_mt_router_info(struct rift_item *i)
{
	struct rift_mt_router_info *info = (struct rift_mt_router_info *)i;
	struct rift_mt_router_info *rv = XCALLOC(MTYPE_RIFT_TLV, sizeof(*rv));
...
static void format_item_mt_router_info(uint16_t mtid, struct rift_item *i,
				       struct sbuf *buf, int indent)
{
	struct rift_mt_router_info *info = (struct rift_mt_router_info *)i;

	sbuf_push(buf, indent, "MT Router Info: %s%s%s\n",
		  rift_mtid2str(info->mtid),
		  info->overload ? " Overload" : "",
		  info->attached ? " Attached" : "");
}

static void free_item_mt_router_info(struct rift_item *i)
{
	XFREE(MTYPE_RIFT_TLV, i);
}

static int pack_item_mt_router_info(struct rift_item *i, struct stream *s)
{
	struct rift_mt_router_info *info = (struct rift_mt_router_info *)i;
	...
	if (info->overload)
		entry |= RIFT_MT_OL_MASK;
	if (info->attached)
		entry |= RIFT_MT_AT_MASK;
				      ...
				      void *dest, int indent)
{
	...
	rv->overload = entry & RIFT_MT_OL_MASK;
	rv->attached = entry & RIFT_MT_AT_MASK;
	rv->mtid = entry & RIFT_MT_MASK;

	format_item_mt_router_info(mtid, (struct rift_item *)rv, log,
				   indent + 2);
	append_item(&tlvs->mt_router_info, (struct rift_item *)rv);
	return 0;
}
...
static struct in_addr *copy_tlv_te_router_id(const struct in_addr *id)
{
	...
	struct in_addr *rv = XCALLOC(MTYPE_RIFT_TLV, sizeof(*rv));
...
static int pack_tlv_te_router_id(const struct in_addr *id, struct stream *s)
{
	...
	stream_putc(s, RIFT_TLV_TE_ROUTER_ID);
	stream_putc(s, 4);
	stream_put(s, id, 4);
	return 0;
}

static int unpack_tlv_te_router_id(enum rift_tlv_context context,
				   uint8_t tlv_type, uint8_t tlv_len,
				   struct stream *s, struct sbuf *log,
				   void *dest, int indent)
{
	...
	tlvs->te_router_id = XCALLOC(MTYPE_RIFT_TLV, 4);
...
static struct rift_item *copy_item_extended_ip_reach(struct rift_item *i)
{
	struct rift_extended_ip_reach *r = (struct rift_extended_ip_reach *)i;
	struct rift_extended_ip_reach *rv =
		XCALLOC(MTYPE_RIFT_TLV, sizeof(*rv));
...
static void format_item_extended_ip_reach(uint16_t mtid, struct rift_item *i,
					  struct sbuf *buf, int indent)
{
	struct rift_extended_ip_reach *r = (struct rift_extended_ip_reach *)i;
	char prefixbuf[PREFIX2STR_BUFFER];
...
static void free_item_extended_ip_reach(struct rift_item *i)
{
	struct rift_extended_ip_reach *item =
		(struct rift_extended_ip_reach *)i;
	rift_free_subtlvs(item->subtlvs);
	XFREE(MTYPE_RIFT_TLV, item);
}

static int pack_item_extended_ip_reach(struct rift_item *i, struct stream *s)
{
	struct rift_extended_ip_reach *r = (struct rift_extended_ip_reach *)i;
	uint8_t control;
	...
	control = r->down ? RIFT_EXTENDED_IP_REACH_DOWN : 0;
	control |= r->prefix.prefixlen;
	control |= r->subtlvs ? RIFT_EXTENDED_IP_REACH_SUBTLV : 0;
...
{
	struct rift_tlvs *tlvs = dest;
	struct rift_extended_ip_reach *rv = NULL;
	size_t consume;
	...
	if (mtid == RIFT_MT_IPV4_UNICAST) {
		...
		items = rift_get_mt_items(&tlvs->mt_ip_reach, mtid);
	}
	...
	rv->down = (control & RIFT_EXTENDED_IP_REACH_DOWN);
	rv->prefix.family = AF_INET;
	...
	format_item_extended_ip_reach(mtid, (struct rift_item *)rv, log,
				      indent + 2);

	if (control & RIFT_EXTENDED_IP_REACH_SUBTLV) {
		consume += 1;
		if (len < consume) {
			sbuf_push(log, indent,
		...
		rv->subtlvs = rift_alloc_subtlvs(RIFT_CONTEXT_SUBTLV_IP_REACH);
		if (unpack_tlvs(RIFT_CONTEXT_SUBTLV_IP_REACH, subtlv_len, s,
...
out:
	if (rv)
		free_item_extended_ip_reach((struct rift_item *)rv);
	return 1;
}
...
static char *copy_tlv_dynamic_hostname(const char *hostname)
{
	...
	return XSTRDUP(MTYPE_RIFT_TLV, hostname);
}
...
static int pack_tlv_dynamic_hostname(const char *hostname, struct stream *s)
{
	...
	stream_putc(s, RIFT_TLV_DYNAMIC_HOSTNAME);
	stream_putc(s, name_len);
	stream_put(s, hostname, name_len);
	return 0;
}

static int unpack_tlv_dynamic_hostname(enum rift_tlv_context context,
				       uint8_t tlv_type, uint8_t tlv_len,
				       struct stream *s, struct sbuf *log,
				       void *dest, int indent)
{
	...
	tlvs->hostname = XCALLOC(MTYPE_RIFT_TLV, tlv_len + 1);
...
static struct rift_spine_leaf *copy_tlv_spine_leaf(
				const struct rift_spine_leaf *spine_leaf)
{
	if (!spine_leaf)
		return NULL;

	struct rift_spine_leaf *rv = XMALLOC(MTYPE_RIFT_TLV, sizeof(*rv));
	memcpy(rv, spine_leaf, sizeof(*rv));

	return rv;
}

static void format_tlv_spine_leaf(const struct rift_spine_leaf *spine_leaf,
				  struct sbuf *buf, int indent)
{
	if (!spine_leaf)
		return;
	...
	if (spine_leaf->has_tier) {
		if (spine_leaf->tier == RIFT_TIER_UNDEFINED) {
...
static void free_tlv_spine_leaf(struct rift_spine_leaf *spine_leaf)
{
	XFREE(MTYPE_RIFT_TLV, spine_leaf);
}

#define RIFT_SPINE_LEAF_FLAG_TIER 0x08
#define RIFT_SPINE_LEAF_FLAG_BACKUP 0x04
#define RIFT_SPINE_LEAF_FLAG_SPINE 0x02
#define RIFT_SPINE_LEAF_FLAG_LEAF 0x01

static int pack_tlv_spine_leaf(const struct rift_spine_leaf *spine_leaf,
			       struct stream *s)
{
	if (!spine_leaf)
		return 0;
	...
	stream_putc(s, RIFT_TLV_SPINE_LEAF_EXT);
	stream_putc(s, tlv_len);

	uint16_t spine_leaf_flags = 0;

	if (spine_leaf->has_tier) {
		spine_leaf_flags |= RIFT_SPINE_LEAF_FLAG_TIER;
		spine_leaf_flags |= spine_leaf->tier << 12;
	}

	if (spine_leaf->is_leaf)
		spine_leaf_flags |= RIFT_SPINE_LEAF_FLAG_LEAF;

	if (spine_leaf->is_spine)
		spine_leaf_flags |= RIFT_SPINE_LEAF_FLAG_SPINE;

	if (spine_leaf->is_backup)
		spine_leaf_flags |= RIFT_SPINE_LEAF_FLAG_BACKUP;
...
static int unpack_tlv_spine_leaf(enum rift_tlv_context context,
				 uint8_t tlv_type, uint8_t tlv_len,
				 struct stream *s, struct sbuf *log,
				 void *dest, int indent)
{
	...
	tlvs->spine_leaf = XCALLOC(MTYPE_RIFT_TLV, sizeof(*tlvs->spine_leaf));

	uint16_t spine_leaf_flags = stream_getw(s);

	if (spine_leaf_flags & RIFT_SPINE_LEAF_FLAG_TIER) {
		tlvs->spine_leaf->has_tier = true;
		tlvs->spine_leaf->tier = spine_leaf_flags >> 12;
	}

	tlvs->spine_leaf->is_leaf = spine_leaf_flags & RIFT_SPINE_LEAF_FLAG_LEAF;
	tlvs->spine_leaf->is_spine = spine_leaf_flags & RIFT_SPINE_LEAF_FLAG_SPINE;
	tlvs->spine_leaf->is_backup = spine_leaf_flags & RIFT_SPINE_LEAF_FLAG_BACKUP;
...
{
	switch (state) {
	case RIFT_THREEWAY_DOWN:
		return "Down";
	case RIFT_THREEWAY_INITIALIZING:
		return "Initializing";
	case RIFT_THREEWAY_UP:
		return "Up";
	default:
		return "Invalid!";
	}
...
static struct rift_threeway_adj *copy_tlv_threeway_adj(
				const struct rift_threeway_adj *threeway_adj)
{
	if (!threeway_adj)
		return NULL;

	struct rift_threeway_adj *rv = XMALLOC(MTYPE_RIFT_TLV, sizeof(*rv));
	memcpy(rv, threeway_adj, sizeof(*rv));

	return rv;
}

static void format_tlv_threeway_adj(const struct rift_threeway_adj *threeway_adj,
				     struct sbuf *buf, int indent)
{
	if (!threeway_adj)
		return;
	...
	sbuf_push(buf, indent, "  State: %s (%d)\n",
		  rift_threeway_state_name(threeway_adj->state),
		  threeway_adj->state);
	...
	sbuf_push(buf, indent, "  Neighbor System ID: %s\n",
		  rift_format_id(threeway_adj->neighbor_id, 6));
	sbuf_push(buf, indent, "  Neighbor Extended Circuit ID: %" PRIu32 "\n",
		  threeway_adj->neighbor_circuit_id);
}

static void free_tlv_threeway_adj(struct rift_threeway_adj *threeway_adj)
{
	XFREE(MTYPE_RIFT_TLV, threeway_adj);
}

static int pack_tlv_threeway_adj(const struct rift_threeway_adj *threeway_adj,
				  struct stream *s)
{
	if (!threeway_adj)
		return 0;
	...
	stream_putc(s, RIFT_TLV_THREE_WAY_ADJ);
	stream_putc(s, tlv_len);
...
static int unpack_tlv_threeway_adj(enum rift_tlv_context context,
				       uint8_t tlv_type, uint8_t tlv_len,
				       struct stream *s, struct sbuf *log,
				       void *dest, int indent)
{
	...
	tlvs->threeway_adj = XCALLOC(MTYPE_RIFT_TLV, sizeof(*tlvs->threeway_adj));
...
static struct rift_item *copy_item_ipv6_reach(struct rift_item *i)
{
	struct rift_ipv6_reach *r = (struct rift_ipv6_reach *)i;
	struct rift_ipv6_reach *rv = XCALLOC(MTYPE_RIFT_TLV, sizeof(*rv));
	rv->metric = r->metric;
	rv->down = r->down;
...
static void format_item_ipv6_reach(uint16_t mtid, struct rift_item *i,
				   struct sbuf *buf, int indent)
{
	struct rift_ipv6_reach *r = (struct rift_ipv6_reach *)i;
	char prefixbuf[PREFIX2STR_BUFFER];
...
static void free_item_ipv6_reach(struct rift_item *i)
{
	struct rift_ipv6_reach *item = (struct rift_ipv6_reach *)i;

	rift_free_subtlvs(item->subtlvs);
	XFREE(MTYPE_RIFT_TLV, item);
}

static int pack_item_ipv6_reach(struct rift_item *i, struct stream *s)
{
	struct rift_ipv6_reach *r = (struct rift_ipv6_reach *)i;
	uint8_t control;
	...
	control = r->down ? RIFT_IPV6_REACH_DOWN : 0;
	control |= r->external ? RIFT_IPV6_REACH_EXTERNAL : 0;
	control |= r->subtlvs ? RIFT_IPV6_REACH_SUBTLV : 0;
...
{
	struct rift_tlvs *tlvs = dest;
	struct rift_ipv6_reach *rv = NULL;
	size_t consume;
	uint8_t control, subtlv_len;
	struct rift_item_list *items;
	...
	if (mtid == RIFT_MT_IPV4_UNICAST) {
		...
		items = rift_get_mt_items(&tlvs->mt_ipv6_reach, mtid);
	}
	...
	rv->metric = stream_getl(s);
	control = stream_getc(s);
	rv->down = (control & RIFT_IPV6_REACH_DOWN);
	rv->external = (control & RIFT_IPV6_REACH_EXTERNAL);
	...
	if (memcmp(&orig_prefix, &rv->prefix.prefix, sizeof(orig_prefix)))
		sbuf_push(log, indent + 2,
			  "WARNING: Prefix had hostbits set.\n");
	format_item_ipv6_reach(mtid, (struct rift_item *)rv, log, indent + 2);

	if (control & RIFT_IPV6_REACH_SUBTLV) {
		consume += 1;
		if (len < consume) {
			sbuf_push(log, indent,
		...
		rv->subtlvs = rift_alloc_subtlvs(RIFT_CONTEXT_SUBTLV_IPV6_REACH);
		if (unpack_tlvs(RIFT_CONTEXT_SUBTLV_IPV6_REACH, subtlv_len, s,
				log, rv->subtlvs, indent + 4)) {
			goto out;
		}
	}
...
out:
	if (rv)
		free_item_ipv6_reach((struct rift_item *)rv);
	return 1;
}

/* Functions related to TLV 10 Authentication */
static struct rift_item *copy_item_auth(struct rift_item *i)
{
	struct rift_auth *auth = (struct rift_auth *)i;
	struct rift_auth *rv = XCALLOC(MTYPE_RIFT_TLV, sizeof(*rv));
...
static void format_item_auth(uint16_t mtid, struct rift_item *i,
			     struct sbuf *buf, int indent)
{
	struct rift_auth *auth = (struct rift_auth *)i;
	char obuf[768];
...
static void free_item_auth(struct rift_item *i)
{
	XFREE(MTYPE_RIFT_TLV, i);
}

static int pack_item_auth(struct rift_item *i, struct stream *s)
{
	struct rift_auth *auth = (struct rift_auth *)i;
			    ...
			    struct sbuf *log, void *dest, int indent)
{
	...
	if (rv->type == RIFT_PASSWD_TYPE_HMAC_MD5 && rv->length != 16) {
		sbuf_push(
			log, indent,
	...
	format_item_auth(mtid, (struct rift_item *)rv, log, indent + 2);
	append_item(&tlvs->rift_auth, (struct rift_item *)rv);
	return 0;
}

/* Functions related to TLV 13 Purge Originator */

static struct rift_purge_originator *copy_tlv_purge_originator(
					struct rift_purge_originator *poi)
{
	if (!poi)
		return NULL;

	struct rift_purge_originator *rv;
...
static void format_tlv_purge_originator(struct rift_purge_originator *poi,
					struct sbuf *buf, int indent)
{
	if (!poi)
		return;
	...
	sbuf_push(buf, indent, "  Generator: %s\n",
		  rift_format_id(poi->generator, sizeof(poi->generator)));
	if (poi->sender_set) {
		sbuf_push(buf, indent, "  Received-From: %s\n",
			  rift_format_id(poi->sender, sizeof(poi->sender)));
	}
}

static void free_tlv_purge_originator(struct rift_purge_originator *poi)
{
	XFREE(MTYPE_RIFT_TLV, poi);
}

static int pack_tlv_purge_originator(struct rift_purge_originator *poi,
				     struct stream *s)
{
	if (!poi)
		return 0;
	...
	stream_putc(s, RIFT_TLV_PURGE_ORIGINATOR);
...
static int unpack_tlv_purge_originator(enum rift_tlv_context context,
				       uint8_t tlv_type, uint8_t tlv_len,
				       struct stream *s, struct sbuf *log,
				       void *dest, int indent)
{
...
static void format_item(uint16_t mtid, enum rift_tlv_context context,
			enum rift_tlv_type type, struct rift_item *i,
			struct sbuf *buf, int indent)
{
	const struct tlv_ops *ops = tlv_table[context][type];
...
static void free_item(enum rift_tlv_context tlv_context,
		      enum rift_tlv_type tlv_type, struct rift_item *item)
{
...
static void free_items(enum rift_tlv_context context, enum rift_tlv_type type,
		       struct rift_item_list *items)
{
	struct rift_item *item, *next_item;
...
static int pack_item(enum rift_tlv_context context, enum rift_tlv_type type,
		     struct rift_item *i, struct stream *s,
		     struct rift_tlvs **fragment_tlvs,
...
static void add_item_to_fragment(struct rift_item *i, struct pack_order_entry *pe,
				 struct rift_tlvs *fragment_tlvs, uint16_t mtid)
{
	struct rift_item_list *l;

	if (pe->how_to_pack == RIFT_ITEMS) {
		l = (struct rift_item_list *)(((char *)fragment_tlvs) + pe->what_to_pack);
	} else {
		struct rift_mt_item_list *m;
		m = (struct rift_mt_item_list *)(((char *)fragment_tlvs) + pe->what_to_pack);
		l = rift_get_mt_items(m, mtid);
	}
...
top:
	...
	if (context == RIFT_CONTEXT_LSP && IS_COMPAT_MT_TLV(type)
	    && mtid != RIFT_MT_IPV4_UNICAST) {
		if (STREAM_WRITEABLE(s) < 2)
			goto too_long;
		stream_putw(s, mtid);
	}

	if (context == RIFT_CONTEXT_LSP && type == RIFT_TLV_OLDSTYLE_REACH) {
		if (STREAM_WRITEABLE(s) < 1)
			goto too_long;
	...
	for (item = item ? item : items->head; item; item = item->next) {
		...
		if (context == RIFT_CONTEXT_LSP && type == RIFT_TLV_AUTH) {
			item = item->next;
			break;
		}

		/* Multiple prefix-sids don't go into one TLV, so always break */
		if (type == RIFT_SUBTLV_PREFIX_SID
		    && (context == RIFT_CONTEXT_SUBTLV_IP_REACH
			|| context == RIFT_CONTEXT_SUBTLV_IPV6_REACH)) {
			item = item->next;
			break;
		}
...
static struct rift_item *last_item(struct rift_item_list *list)
{
	return container_of(list->tail, struct rift_item, next);
}

static int unpack_item(uint16_t mtid, enum rift_tlv_context context,
...
static int unpack_tlv_with_items(enum rift_tlv_context context,
				 uint8_t tlv_type, uint8_t tlv_len,
				 ...
				 int indent)
{
	...
	if (context == RIFT_CONTEXT_LSP && IS_COMPAT_MT_TLV(tlv_type)) {
		if (tlv_len < 2) {
			sbuf_push(log, indent,
				  "TLV is too short to contain MTID\n");
			return 1;
		}
		mtid = stream_getw(s) & RIFT_MT_MASK;
		tlv_pos += 2;
	...
	if (context == RIFT_CONTEXT_LSP
	    && tlv_type == RIFT_TLV_OLDSTYLE_REACH) {
	...
	if (context == RIFT_CONTEXT_LSP
	    && tlv_type == RIFT_TLV_OLDSTYLE_IP_REACH) {
		struct rift_tlvs *tlvs = dest;
		dest = &tlvs->oldstyle_ip_reach;
	} else if (context == RIFT_CONTEXT_LSP
		   && tlv_type == RIFT_TLV_OLDSTYLE_IP_REACH_EXT) {
		struct rift_tlvs *tlvs = dest;
		dest = &tlvs->oldstyle_ip_reach_ext;
	}

	if (context == RIFT_CONTEXT_LSP
	    && tlv_type == RIFT_TLV_MT_ROUTER_INFO) {
		struct rift_tlvs *tlvs = dest;
...
static int rift_mt_item_list_cmp(const struct rift_item_list *a,
				 const struct rift_item_list *b)
{
	if (a->mtid < b->mtid)
		return -1;
	if (a->mtid > b->mtid)
...
RB_PROTOTYPE(rift_mt_item_list, rift_item_list, mt_tree, rift_mt_item_list_cmp);
RB_GENERATE(rift_mt_item_list, rift_item_list, mt_tree, rift_mt_item_list_cmp);

struct rift_item_list *rift_get_mt_items(struct rift_mt_item_list *m,
					 uint16_t mtid)
{
	struct rift_item_list *rv;

	rv = rift_lookup_mt_items(m, mtid);
	if (!rv) {
		rv = XCALLOC(MTYPE_RIFT_MT_ITEM_LIST, sizeof(*rv));
		init_item_list(rv);
		rv->mtid = mtid;
		RB_INSERT(rift_mt_item_list, m, rv);
	}
...
struct rift_item_list *rift_lookup_mt_items(struct rift_mt_item_list *m,
					    uint16_t mtid)
{
	struct rift_item_list key = {.mtid = mtid};

	return RB_FIND(rift_mt_item_list, m, &key);
}

static void free_mt_items(enum rift_tlv_context context,
			  enum rift_tlv_type type, struct rift_mt_item_list *m)
{
	struct rift_item_list *n, *nnext;

	RB_FOREACH_SAFE (n, rift_mt_item_list, m, nnext) {
		free_items(context, type, n);
		RB_REMOVE(rift_mt_item_list, m, n);
		XFREE(MTYPE_RIFT_MT_ITEM_LIST, n);
	}
}

static void format_mt_items(enum rift_tlv_context context,
			    enum rift_tlv_type type,
			    struct rift_mt_item_list *m, struct sbuf *buf,
			    int indent)
{
	struct rift_item_list *n;

	RB_FOREACH (n, rift_mt_item_list, m) {
		format_items_(n->mtid, context, type, n, buf, indent);
	}
}

static int pack_mt_items(enum rift_tlv_context context, enum rift_tlv_type type,
			 struct rift_mt_item_list *m, struct stream *s,
			 struct rift_tlvs **fragment_tlvs,
			 struct pack_order_entry *pe,
			 struct rift_tlvs *(*new_fragment)(struct list *l),
			 struct list *new_fragment_arg)
...
static void copy_mt_items(enum rift_tlv_context context,
			  enum rift_tlv_type type,
			  struct rift_mt_item_list *src,
			  struct rift_mt_item_list *dest)
{
	struct rift_item_list *n;

	RB_INIT(rift_mt_item_list, dest);

	RB_FOREACH (n, rift_mt_item_list, src) {
		copy_items(context, type, n, rift_get_mt_items(dest, n->mtid));
	}
}
...
{
	struct rift_tlvs *result;

	result = XCALLOC(MTYPE_RIFT_TLV, sizeof(*result));

	init_item_list(&result->rift_auth);
	...
	RB_INIT(rift_mt_item_list, &result->mt_reach);
	...
	RB_INIT(rift_mt_item_list, &result->mt_ip_reach);
	init_item_list(&result->ipv6_reach);
	RB_INIT(rift_mt_item_list, &result->mt_ipv6_reach);
...
{
	struct rift_tlvs *rv = XCALLOC(MTYPE_RIFT_TLV, sizeof(*rv));

	copy_items(RIFT_CONTEXT_LSP, RIFT_TLV_AUTH, &tlvs->rift_auth,
		   &rv->rift_auth);

	rv->purge_originator =
			copy_tlv_purge_originator(tlvs->purge_originator);

	copy_items(RIFT_CONTEXT_LSP, RIFT_TLV_AREA_ADDRESSES,
		   &tlvs->area_addresses, &rv->area_addresses);

	copy_items(RIFT_CONTEXT_LSP, RIFT_TLV_MT_ROUTER_INFO,
		   &tlvs->mt_router_info, &rv->mt_router_info);

	rv->mt_router_info_empty = tlvs->mt_router_info_empty;

	copy_items(RIFT_CONTEXT_LSP, RIFT_TLV_OLDSTYLE_REACH,
		   &tlvs->oldstyle_reach, &rv->oldstyle_reach);

	copy_items(RIFT_CONTEXT_LSP, RIFT_TLV_LAN_NEIGHBORS,
		   &tlvs->lan_neighbor, &rv->lan_neighbor);

	copy_items(RIFT_CONTEXT_LSP, RIFT_TLV_LSP_ENTRY, &tlvs->lsp_entries,
		   &rv->lsp_entries);

	copy_items(RIFT_CONTEXT_LSP, RIFT_TLV_EXTENDED_REACH,
		   &tlvs->extended_reach, &rv->extended_reach);

	copy_mt_items(RIFT_CONTEXT_LSP, RIFT_TLV_MT_REACH, &tlvs->mt_reach,
		      &rv->mt_reach);

	copy_items(RIFT_CONTEXT_LSP, RIFT_TLV_OLDSTYLE_IP_REACH,
		   &tlvs->oldstyle_ip_reach, &rv->oldstyle_ip_reach);

	copy_tlv_protocols_supported(&tlvs->protocols_supported,
				     &rv->protocols_supported);

	copy_items(RIFT_CONTEXT_LSP, RIFT_TLV_OLDSTYLE_IP_REACH_EXT,
		   &tlvs->oldstyle_ip_reach_ext, &rv->oldstyle_ip_reach_ext);

	copy_items(RIFT_CONTEXT_LSP, RIFT_TLV_IPV4_ADDRESS, &tlvs->ipv4_address,
		   &rv->ipv4_address);

	copy_items(RIFT_CONTEXT_LSP, RIFT_TLV_IPV6_ADDRESS, &tlvs->ipv6_address,
		   &rv->ipv6_address);

	rv->te_router_id = copy_tlv_te_router_id(tlvs->te_router_id);

	copy_items(RIFT_CONTEXT_LSP, RIFT_TLV_EXTENDED_IP_REACH,
		   &tlvs->extended_ip_reach, &rv->extended_ip_reach);

	copy_mt_items(RIFT_CONTEXT_LSP, RIFT_TLV_MT_IP_REACH,
		      &tlvs->mt_ip_reach, &rv->mt_ip_reach);

	rv->hostname = copy_tlv_dynamic_hostname(tlvs->hostname);

	copy_items(RIFT_CONTEXT_LSP, RIFT_TLV_IPV6_REACH, &tlvs->ipv6_reach,
		   &rv->ipv6_reach);

	copy_mt_items(RIFT_CONTEXT_LSP, RIFT_TLV_MT_IPV6_REACH,
...
static void format_tlvs(struct rift_tlvs *tlvs, struct sbuf *buf, int indent)
{
	format_tlv_protocols_supported(&tlvs->protocols_supported, buf, indent);

	format_items(RIFT_CONTEXT_LSP, RIFT_TLV_AUTH, &tlvs->rift_auth, buf,
		     indent);

	format_tlv_purge_originator(tlvs->purge_originator, buf, indent);

	format_items(RIFT_CONTEXT_LSP, RIFT_TLV_AREA_ADDRESSES,
		     &tlvs->area_addresses, buf, indent);

	if (tlvs->mt_router_info_empty) {
		sbuf_push(buf, indent, "MT Router Info: None\n");
	} else {
		format_items(RIFT_CONTEXT_LSP, RIFT_TLV_MT_ROUTER_INFO,
			     &tlvs->mt_router_info, buf, indent);
	}

	format_items(RIFT_CONTEXT_LSP, RIFT_TLV_OLDSTYLE_REACH,
		     &tlvs->oldstyle_reach, buf, indent);

	format_items(RIFT_CONTEXT_LSP, RIFT_TLV_LAN_NEIGHBORS,
		     &tlvs->lan_neighbor, buf, indent);

	format_items(RIFT_CONTEXT_LSP, RIFT_TLV_LSP_ENTRY, &tlvs->lsp_entries,
		     buf, indent);

	format_tlv_dynamic_hostname(tlvs->hostname, buf, indent);
	format_tlv_te_router_id(tlvs->te_router_id, buf, indent);

	format_items(RIFT_CONTEXT_LSP, RIFT_TLV_EXTENDED_REACH,
		     &tlvs->extended_reach, buf, indent);

	format_mt_items(RIFT_CONTEXT_LSP, RIFT_TLV_MT_REACH, &tlvs->mt_reach,
			buf, indent);

	format_items(RIFT_CONTEXT_LSP, RIFT_TLV_OLDSTYLE_IP_REACH,
		     &tlvs->oldstyle_ip_reach, buf, indent);

	format_items(RIFT_CONTEXT_LSP, RIFT_TLV_OLDSTYLE_IP_REACH_EXT,
		     &tlvs->oldstyle_ip_reach_ext, buf, indent);

	format_items(RIFT_CONTEXT_LSP, RIFT_TLV_IPV4_ADDRESS,
		     &tlvs->ipv4_address, buf, indent);

	format_items(RIFT_CONTEXT_LSP, RIFT_TLV_IPV6_ADDRESS,
		     &tlvs->ipv6_address, buf, indent);

	format_items(RIFT_CONTEXT_LSP, RIFT_TLV_EXTENDED_IP_REACH,
		     &tlvs->extended_ip_reach, buf, indent);

	format_mt_items(RIFT_CONTEXT_LSP, RIFT_TLV_MT_IP_REACH,
			&tlvs->mt_ip_reach, buf, indent);

	format_items(RIFT_CONTEXT_LSP, RIFT_TLV_IPV6_REACH, &tlvs->ipv6_reach,
		     buf, indent);

	format_mt_items(RIFT_CONTEXT_LSP, RIFT_TLV_MT_IPV6_REACH,
...
void rift_free_tlvs(struct rift_tlvs *tlvs)
{
	...
	free_items(RIFT_CONTEXT_LSP, RIFT_TLV_AUTH, &tlvs->rift_auth);
	free_tlv_purge_originator(tlvs->purge_originator);
	free_items(RIFT_CONTEXT_LSP, RIFT_TLV_AREA_ADDRESSES,
		   &tlvs->area_addresses);
	free_items(RIFT_CONTEXT_LSP, RIFT_TLV_MT_ROUTER_INFO,
		   &tlvs->mt_router_info);
	free_items(RIFT_CONTEXT_LSP, RIFT_TLV_OLDSTYLE_REACH,
		   &tlvs->oldstyle_reach);
	free_items(RIFT_CONTEXT_LSP, RIFT_TLV_LAN_NEIGHBORS,
		   &tlvs->lan_neighbor);
	free_items(RIFT_CONTEXT_LSP, RIFT_TLV_LSP_ENTRY, &tlvs->lsp_entries);
	free_items(RIFT_CONTEXT_LSP, RIFT_TLV_EXTENDED_REACH,
		   &tlvs->extended_reach);
	free_mt_items(RIFT_CONTEXT_LSP, RIFT_TLV_MT_REACH, &tlvs->mt_reach);
	free_items(RIFT_CONTEXT_LSP, RIFT_TLV_OLDSTYLE_IP_REACH,
		   &tlvs->oldstyle_ip_reach);
	free_tlv_protocols_supported(&tlvs->protocols_supported);
	free_items(RIFT_CONTEXT_LSP, RIFT_TLV_OLDSTYLE_IP_REACH_EXT,
		   &tlvs->oldstyle_ip_reach_ext);
	free_items(RIFT_CONTEXT_LSP, RIFT_TLV_IPV4_ADDRESS,
		   &tlvs->ipv4_address);
	free_items(RIFT_CONTEXT_LSP, RIFT_TLV_IPV6_ADDRESS,
		   &tlvs->ipv6_address);
	free_tlv_te_router_id(tlvs->te_router_id);
	free_items(RIFT_CONTEXT_LSP, RIFT_TLV_EXTENDED_IP_REACH,
		   &tlvs->extended_ip_reach);
	free_mt_items(RIFT_CONTEXT_LSP, RIFT_TLV_MT_IP_REACH,
		      &tlvs->mt_ip_reach);
	free_tlv_dynamic_hostname(tlvs->hostname);
	free_items(RIFT_CONTEXT_LSP, RIFT_TLV_IPV6_REACH, &tlvs->ipv6_reach);
	free_mt_items(RIFT_CONTEXT_LSP, RIFT_TLV_MT_IPV6_REACH,
		      &tlvs->mt_ipv6_reach);
	free_tlv_threeway_adj(tlvs->threeway_adj);
	free_tlv_spine_leaf(tlvs->spine_leaf);

	XFREE(MTYPE_RIFT_TLV, tlvs);
}
...
static void add_padding(struct stream *s)
{
	while (STREAM_WRITEABLE(s)) {
		...
		stream_putc(s, RIFT_TLV_PADDING);
...
static void update_auth_hmac_md5(struct rift_auth *auth, struct stream *s,
				 bool is_lsp)
{
	uint8_t digest[16];
...
static void update_auth(struct rift_tlvs *tlvs, struct stream *s, bool is_lsp)
{
	struct rift_auth *auth_head = (struct rift_auth *)tlvs->rift_auth.head;

	for (struct rift_auth *auth = auth_head; auth; auth = auth->next) {
		if (auth->type == RIFT_PASSWD_TYPE_HMAC_MD5)
			     ...
			     struct list *new_fragment_arg)
{
	...
	if (pe->how_to_pack == RIFT_ITEMS) {
		struct rift_item_list *l;
		l = (struct rift_item_list *)(((char *)tlvs)
					      + pe->what_to_pack);
		rv = pack_items(pe->context, pe->type, l, stream, fragment_tlvs,
				pe, new_fragment, new_fragment_arg);
	} else {
		struct rift_mt_item_list *l;
		l = (struct rift_mt_item_list *)(((char *)tlvs)
						 + pe->what_to_pack);
...
static int pack_tlvs(struct rift_tlvs *tlvs, struct stream *stream,
		     struct rift_tlvs *fragment_tlvs,
		     struct rift_tlvs *(*new_fragment)(struct list *l),
		     struct list *new_fragment_arg)
{
	 ...
	 * size we are given. */
	if (!fragment_tlvs) {
		rv = pack_items(RIFT_CONTEXT_LSP, RIFT_TLV_AUTH,
				&tlvs->rift_auth, stream, NULL, NULL, NULL,
				NULL);
		if (rv)
			return rv;
	}
	...
	rv = pack_items(RIFT_CONTEXT_LSP, RIFT_TLV_AREA_ADDRESSES,
	...
	if (tlvs->mt_router_info_empty) {
		if (STREAM_WRITEABLE(stream) < 2)
			return 1;
		stream_putc(stream, RIFT_TLV_MT_ROUTER_INFO);
		stream_putc(stream, 0);
		if (fragment_tlvs)
			fragment_tlvs->mt_router_info_empty = true;
	} else {
		rv = pack_items(RIFT_CONTEXT_LSP, RIFT_TLV_MT_ROUTER_INFO,
...
static struct rift_tlvs *new_fragment(struct list *l)
{
	struct rift_tlvs *rv = rift_alloc_tlvs();
...
static int unpack_tlv_unknown(enum rift_tlv_context context, uint8_t tlv_type,
			      uint8_t tlv_len, struct stream *s,
...
static int unpack_tlv(enum rift_tlv_context context, size_t avail_len,
		      struct stream *stream, struct sbuf *log, void *dest,
		      int indent)
{
		       ...
		       int indent)
{
	...
	sbuf_push(log, indent, "Unpacking %zu bytes of %s...\n", avail_len,
		  (context == RIFT_CONTEXT_LSP) ? "TLVs" : "sub-TLVs");
...
int rift_unpack_tlvs(size_t avail_len, struct stream *stream,
		     struct rift_tlvs **dest, const char **log)
{
	...
	result = rift_alloc_tlvs();
	rv = unpack_tlvs(RIFT_CONTEXT_LSP, avail_len, stream, &logbuf, result,
			 indent);
...
ITEM_TLV_OPS(auth, "TLV 10 RIFT Auth");
...
static const struct tlv_ops *tlv_table[RIFT_CONTEXT_MAX][RIFT_TLV_MAX] = {
	[RIFT_CONTEXT_LSP] = {
		[RIFT_TLV_AREA_ADDRESSES] = &tlv_area_address_ops,
		[RIFT_TLV_OLDSTYLE_REACH] = &tlv_oldstyle_reach_ops,
		[RIFT_TLV_LAN_NEIGHBORS] = &tlv_lan_neighbor_ops,
		[RIFT_TLV_LSP_ENTRY] = &tlv_lsp_entry_ops,
		[RIFT_TLV_AUTH] = &tlv_auth_ops,
		[RIFT_TLV_PURGE_ORIGINATOR] = &tlv_purge_originator_ops,
		[RIFT_TLV_EXTENDED_REACH] = &tlv_extended_reach_ops,
		[RIFT_TLV_MT_REACH] = &tlv_extended_reach_ops,
		[RIFT_TLV_OLDSTYLE_IP_REACH] = &tlv_oldstyle_ip_reach_ops,
		[RIFT_TLV_PROTOCOLS_SUPPORTED] = &tlv_protocols_supported_ops,
		[RIFT_TLV_OLDSTYLE_IP_REACH_EXT] = &tlv_oldstyle_ip_reach_ops,
		[RIFT_TLV_IPV4_ADDRESS] = &tlv_ipv4_address_ops,
		[RIFT_TLV_TE_ROUTER_ID] = &tlv_te_router_id_ops,
		[RIFT_TLV_EXTENDED_IP_REACH] = &tlv_extended_ip_reach_ops,
		[RIFT_TLV_MT_IP_REACH] = &tlv_extended_ip_reach_ops,
		[RIFT_TLV_DYNAMIC_HOSTNAME] = &tlv_dynamic_hostname_ops,
		[RIFT_TLV_SPINE_LEAF_EXT] = &tlv_spine_leaf_ops,
		[RIFT_TLV_MT_ROUTER_INFO] = &tlv_mt_router_info_ops,
		[RIFT_TLV_THREE_WAY_ADJ] = &tlv_threeway_adj_ops,
		[RIFT_TLV_IPV6_ADDRESS] = &tlv_ipv6_address_ops,
		[RIFT_TLV_IPV6_REACH] = &tlv_ipv6_reach_ops,
		[RIFT_TLV_MT_IPV6_REACH] = &tlv_ipv6_reach_ops,
	},
	[RIFT_CONTEXT_SUBTLV_NE_REACH] = {},
	[RIFT_CONTEXT_SUBTLV_IP_REACH] = {
		[RIFT_SUBTLV_PREFIX_SID] = &tlv_prefix_sid_ops,
	},
	[RIFT_CONTEXT_SUBTLV_IPV6_REACH] = {
		[RIFT_SUBTLV_PREFIX_SID] = &tlv_prefix_sid_ops,
		[RIFT_SUBTLV_IPV6_SOURCE_PREFIX] = &subtlv_ipv6_source_prefix_ops,
	}
};
...
{
	free_items(RIFT_CONTEXT_LSP, RIFT_TLV_AUTH, &tlvs->rift_auth);
	init_item_list(&tlvs->rift_auth);

	if (passwd->type == RIFT_PASSWD_TYPE_UNUSED)
		return;

	struct rift_auth *auth = XCALLOC(MTYPE_RIFT_TLV, sizeof(*auth));
				  ...
				  struct list *addresses)
{
	...
	for (ALL_LIST_ELEMENTS_RO(addresses, node, area_addr)) {
		struct rift_area_address *a =
			XCALLOC(MTYPE_RIFT_TLV, sizeof(*a));

		a->len = area_addr->addr_len;
		memcpy(a->addr, area_addr->area_addr, 20);
		append_item(&tlvs->area_addresses, (struct rift_item *)a);
	}
}

void rift_tlvs_add_lan_neighbors(struct rift_tlvs *tlvs, struct list *neighbors)
{
	struct listnode *node;
	uint8_t *snpa;

	for (ALL_LIST_ELEMENTS_RO(neighbors, node, snpa)) {
		struct rift_lan_neighbor *n =
			XCALLOC(MTYPE_RIFT_TLV, sizeof(*n));

		memcpy(n->mac, snpa, 6);
		append_item(&tlvs->lan_neighbor, (struct rift_item *)n);
	}
}
				       ...
				       struct nlpids *nlpids)
{
	...
	XFREE(MTYPE_RIFT_TLV, tlvs->protocols_supported.protocols);
	if (nlpids->count) {
		tlvs->protocols_supported.protocols =
			XCALLOC(MTYPE_RIFT_TLV, nlpids->count);
...
{
	struct rift_mt_router_info *i = XCALLOC(MTYPE_RIFT_TLV, sizeof(*i));

	i->overload = overload;
	i->attached = attached;
	i->mtid = mtid;
	append_item(&tlvs->mt_router_info, (struct rift_item *)i);
}

void rift_tlvs_add_ipv4_address(struct rift_tlvs *tlvs, struct in_addr *addr)
{
	struct rift_ipv4_address *a = XCALLOC(MTYPE_RIFT_TLV, sizeof(*a));
	a->addr = *addr;
	append_item(&tlvs->ipv4_address, (struct rift_item *)a);
}
				  ...
				  struct list *addresses)
{
	...
	for (ALL_LIST_ELEMENTS_RO(addresses, node, ip_addr)) {
		rift_tlvs_add_ipv4_address(tlvs, &ip_addr->prefix);
		addr_count++;
		if (addr_count >= 63)
			break;
	}
				  ...
				  struct list *addresses)
{
	...
	for (ALL_LIST_ELEMENTS_RO(addresses, node, ip_addr)) {
		struct rift_ipv6_address *a =
			XCALLOC(MTYPE_RIFT_TLV, sizeof(*a));

		a->addr = ip_addr->prefix;
		append_item(&tlvs->ipv6_address, (struct rift_item *)a);
	}
}

typedef bool (*auth_validator_func)(struct rift_passwd *passwd,
				    struct stream *stream,
				    struct rift_auth *auth, bool is_lsp);

static bool auth_validator_cleartxt(struct rift_passwd *passwd,
				    struct stream *stream,
				    struct rift_auth *auth, bool is_lsp)
{
...
static bool auth_validator_hmac_md5(struct rift_passwd *passwd,
				    struct stream *stream,
				    struct rift_auth *auth, bool is_lsp)
{
	uint8_t digest[16];
...
static const auth_validator_func auth_validators[] = {
		[RIFT_PASSWD_TYPE_CLEARTXT] = auth_validator_cleartxt,
		[RIFT_PASSWD_TYPE_HMAC_MD5] = auth_validator_hmac_md5,
};
			    ...
			    struct stream *stream, bool is_lsp)
{
	...
	if (passwd->type >= array_size(auth_validators)
		...
		return RIFT_AUTH_NO_VALIDATOR;

	struct rift_auth *auth_head = (struct rift_auth *)tlvs->rift_auth.head;
	struct rift_auth *auth;
	...
	if (auth_validators[passwd->type](passwd, stream, auth, is_lsp))
		...
		return RIFT_AUTH_FAILURE;
}

bool rift_tlvs_area_addresses_match(struct rift_tlvs *tlvs,
				    struct list *addresses)
{
	struct rift_area_address *addr_head;

	addr_head = (struct rift_area_address *)tlvs->area_addresses.head;
	for (struct rift_area_address *addr = addr_head; addr;
	     addr = addr->next) {
		struct listnode *node;
		struct area_addr *a;
...
static void tlvs_area_addresses_to_adj(struct rift_tlvs *tlvs,
	...
	if (adj->area_address_count != tlvs->area_addresses.count) {
		*changed = true;
		...
		adj->area_addresses = XREALLOC(
			MTYPE_RIFT_ADJACENCY_INFO, adj->area_addresses,
			adj->area_address_count * sizeof(*adj->area_addresses));
	}

	struct rift_area_address *addr = NULL;
	for (unsigned int i = 0; i < tlvs->area_addresses.count; i++) {
		if (!addr)
			addr = (struct rift_area_address *)
...
static void tlvs_protocols_supported_to_adj(struct rift_tlvs *tlvs,
					    struct rift_adjacency *adj,
					    bool *changed)
{
...
static void tlvs_ipv4_addresses_to_adj(struct rift_tlvs *tlvs,
				       struct rift_adjacency *adj,
				       bool *changed)
{
	if (adj->ipv4_address_count != tlvs->ipv4_address.count) {
		*changed = true;
		...
		adj->ipv4_addresses = XREALLOC(
			MTYPE_RIFT_ADJACENCY_INFO, adj->ipv4_addresses,
			adj->ipv4_address_count * sizeof(*adj->ipv4_addresses));
	}

	struct rift_ipv4_address *addr = NULL;
	for (unsigned int i = 0; i < tlvs->ipv4_address.count; i++) {
		if (!addr)
			addr = (struct rift_ipv4_address *)
...
static void tlvs_ipv6_addresses_to_adj(struct rift_tlvs *tlvs,
				       struct rift_adjacency *adj,
				       bool *changed)
{
	if (adj->ipv6_address_count != tlvs->ipv6_address.count) {
		*changed = true;
		...
		adj->ipv6_addresses = XREALLOC(
			MTYPE_RIFT_ADJACENCY_INFO, adj->ipv6_addresses,
			adj->ipv6_address_count * sizeof(*adj->ipv6_addresses));
	}

	struct rift_ipv6_address *addr = NULL;
	for (unsigned int i = 0; i < tlvs->ipv6_address.count; i++) {
		if (!addr)
			addr = (struct rift_ipv6_address *)
...
{
	struct rift_lan_neighbor *ne_head;

	ne_head = (struct rift_lan_neighbor *)tlvs->lan_neighbor.head;
	for (struct rift_lan_neighbor *ne = ne_head; ne; ne = ne->next) {
...
{
	struct rift_lsp_entry *entry = XCALLOC(MTYPE_RIFT_TLV, sizeof(*entry));

	entry->rem_lifetime = lsp->hdr.rem_lifetime;
	memcpy(entry->id, lsp->hdr.lsp_id, RIFT_SYS_ID_LEN + 2);
	entry->checksum = lsp->hdr.checksum;
	entry->seqno = lsp->hdr.seqno;
	entry->lsp = lsp;

	append_item(&tlvs->lsp_entries, (struct rift_item *)entry);
}
				...
				dict_t *lspdb, struct rift_lsp **last_lsp)
{
	...
	rift_tlvs_add_lsp_entry(tlvs, first->dict_data);
	*last_lsp = first->dict_data;

	while (curr) {
		curr = dict_next(lspdb, curr);
		if (curr) {
			rift_tlvs_add_lsp_entry(tlvs, curr->dict_data);
...
void rift_tlvs_set_dynamic_hostname(struct rift_tlvs *tlvs,
				    const char *hostname)
{
	XFREE(MTYPE_RIFT_TLV, tlvs->hostname);
	if (hostname)
		tlvs->hostname = XSTRDUP(MTYPE_RIFT_TLV, hostname);
}

void rift_tlvs_set_te_router_id(struct rift_tlvs *tlvs,
				const struct in_addr *id)
{
	XFREE(MTYPE_RIFT_TLV, tlvs->te_router_id);
	if (!id)
		return;
	tlvs->te_router_id = XCALLOC(MTYPE_RIFT_TLV, sizeof(*id));
...
{
	struct rift_oldstyle_ip_reach *r = XCALLOC(MTYPE_RIFT_TLV, sizeof(*r));

	r->metric = metric;
	memcpy(&r->prefix, dest, sizeof(*dest));
	apply_mask_ipv4(&r->prefix);
	append_item(&tlvs->oldstyle_ip_reach, (struct rift_item *)r);
}

void rift_tlvs_add_extended_ip_reach(struct rift_tlvs *tlvs,
				     struct prefix_ipv4 *dest, uint32_t metric)
{
	struct rift_extended_ip_reach *r = XCALLOC(MTYPE_RIFT_TLV, sizeof(*r));

	r->metric = metric;
	memcpy(&r->prefix, dest, sizeof(*dest));
	apply_mask_ipv4(&r->prefix);
	append_item(&tlvs->extended_ip_reach, (struct rift_item *)r);
}

void rift_tlvs_add_ipv6_reach(struct rift_tlvs *tlvs, uint16_t mtid,
			      struct prefix_ipv6 *dest, uint32_t metric)
{
	struct rift_ipv6_reach *r = XCALLOC(MTYPE_RIFT_TLV, sizeof(*r));
	...
	struct rift_item_list *l;
	l = (mtid == RIFT_MT_IPV4_UNICAST)
		    ? &tlvs->ipv6_reach
		    : rift_get_mt_items(&tlvs->mt_ipv6_reach, mtid);
	append_item(l, (struct rift_item *)r);
}
...
void rift_tlvs_add_ipv6_dstsrc_reach(struct rift_tlvs *tlvs, uint16_t mtid,
				     ...
				     uint32_t metric)
{
	rift_tlvs_add_ipv6_reach(tlvs, mtid, dest, metric);
	struct rift_item_list *l = rift_get_mt_items(&tlvs->mt_ipv6_reach,
						     mtid);

	struct rift_ipv6_reach *r = (struct rift_ipv6_reach*)last_item(l);
	r->subtlvs = rift_alloc_subtlvs(RIFT_CONTEXT_SUBTLV_IPV6_REACH);
	r->subtlvs->source_prefix = XCALLOC(MTYPE_RIFT_SUBTLV, sizeof(*src));
...
void rift_tlvs_add_oldstyle_reach(struct rift_tlvs *tlvs, uint8_t *id,
				  uint8_t metric)
{
	struct rift_oldstyle_reach *r = XCALLOC(MTYPE_RIFT_TLV, sizeof(*r));

	r->metric = metric;
	memcpy(r->id, id, sizeof(r->id));
	append_item(&tlvs->oldstyle_reach, (struct rift_item *)r);
}
...
{
	struct rift_extended_reach *r = XCALLOC(MTYPE_RIFT_TLV, sizeof(*r));

	memcpy(r->id, id, sizeof(r->id));
	r->metric = metric;
	if (subtlvs && subtlv_len) {
		r->subtlvs = XCALLOC(MTYPE_RIFT_TLV, subtlv_len);
	...
	if (mtid == RIFT_MT_IPV4_UNICAST)
		...
		l = rift_get_mt_items(&tlvs->mt_reach, mtid);
	append_item(l, (struct rift_item *)r);
}
...
rift_tlvs_lookup_mt_router_info(struct rift_tlvs *tlvs, uint16_t mtid)
{
	...
	struct rift_mt_router_info *rv;
	for (rv = (struct rift_mt_router_info *)tlvs->mt_router_info.head; rv;
	     rv = rv->next) {
		if (rv->mtid == mtid)
			return rv;
	}
				    ...
				    const uint8_t *sender)
{
	...
	tlvs->purge_originator = XCALLOC(MTYPE_RIFT_TLV,
