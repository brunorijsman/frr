...
{
	dict_t *new = XCALLOC(MTYPE_RIFT_DICT, sizeof(dict_t));
...
static dnode_t *dnode_alloc(void *context)
{
	return XCALLOC(MTYPE_RIFT_DICT_NODE, sizeof(dnode_t));
}

static void dnode_free(dnode_t *node, void *context)
{
	XFREE(MTYPE_RIFT_DICT_NODE, node);
}

dnode_t *dnode_create(void *data)
{
	dnode_t *new = XCALLOC(MTYPE_RIFT_DICT_NODE, sizeof(dnode_t));
...
void dnode_destroy(dnode_t *dnode)
{
	...
	XFREE(MTYPE_RIFT_DICT_NODE, dnode);
}
...
{
	int sz = strlen(str) + 1;
	char *new = XCALLOC(MTYPE_RIFT_TMP, sz);
